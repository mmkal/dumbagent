...
Status: first slice implemented. TUI UI now launches OpenCode with a deterministic local server port, exposes SDK-backed YAML in the Summary tab, and can trigger OpenCode's own session summarize/compact action. Remaining work is to deepen this into non-mutating AI summary UX and add Codex/Claude adapters.
...
- [x] Implement OpenCode as the first provider using its server SDK/API. _`refreshSessionSdk` reads provider data and `summarizeSessionWithSdk` calls OpenCode `session.summarize` via `@opencode-ai/sdk/client`; prompt input stays on the PTY path for now._
...
- 2026-05-08: Session IDs are now `tuiui_` plus a hyphenless UUID so provider/test scratch space is easier to identify. The Summary tab exposes `sidecarSummary` as YAML and uses OpenCode `session.summarize` as the current sidecar implementation, with a note that this mutates/compacts the provider session.
