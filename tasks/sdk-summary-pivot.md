...
Status: Codex adapter slice implemented. TUI UI now launches OpenCode with a deterministic local server port, resolves Codex threads from local state, exposes provider-backed YAML in the Summary tab, and can trigger non-mutating sidecar summaries for OpenCode and Codex. Remaining work is to deepen this into richer AI summary UX and add Claude.
